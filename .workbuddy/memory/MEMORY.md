...
- 场景 (v0.3.0): 玩家在行驶的地铁【车顶】上跑, 【车顶表面 = y=0】(物理逻辑不变的关键约定)
  - 车厢挂在 y<0 (发光车窗/色带), 隧道地面 y≈-3.4, 墙在 WALL_X=±5.2 (两侧有落差)
  - 车顶细节: 防滑沿/车厢接缝(每15m)/走线槽(兼车道分隔线)
  - 障碍: 路障(跳)/悬空横梁(铲)/设备机房housing(绕, 带警示灯+格栅+黄色警示带)
